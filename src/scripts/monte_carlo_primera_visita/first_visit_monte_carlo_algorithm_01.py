# -*- coding: utf-8 -*-
"""first_visit_monte_carlo_algorithm_01.ipynb
...
# =====================================================================
# Monte Carlo (MC) de Primera Visita
#
# =====================================================================
...
Dada una cuadricula que representa un laberinto el objetivo es estimar qué tan
bueno es estar en cada casilla (estado) del laberinto para el Buscador, usando
una política de movimiento fija, basándose solo en la experiencia de varios
intentos (episodios).
...
Política (π) Fija (desde cualquier casilla no terminal):

      60% de probabilidad de intentar moverse a la DERECHA.
      40% de probabilidad de intentar moverse ABAJO.
      Si el movimiento elegido lo saca del laberinto (choca con un muro
      imaginario), se queda en la misma casilla para ese paso.
...
Factor de Descuento: γ = 1 (sin descuento, para simplificar). El valor es la
...
# Initialize V(s) for all states (including terminals for simplicity, though unused)
...
print(f"Running First-Visit Monte Carlo for {NUM_EPISODES} episodes...")
...
for i in range(NUM_EPISODES):
    ...
    states_visited_in_episode = set() # Track first visits within this episode
    ...
    for t in range(len(episode) - 1, -1, -1):
        ...
        # 3. Check if this is the first visit to S_t in *this* episode
        if state not in states_visited_in_episode:
            # Update V(S_t) using the calculated return G
            returns_sum[state] += G
            returns_count[state] += 1
            # The value is the average of returns seen so far after first visits
            V[state] = returns_sum[state] / returns_count[state]

            # Mark state as visited in this episode
...
# --- Output Results ---
print("\n--- Final State Values (First-Visit MC) ---")
